...
Output is a **tab-separated TSV** (opens directly in Excel) with columns `symbol · gene_id · name · co_papers · gene_papers · specificity · spec_adj · below_floor · artifact · evidence_pmids · ot_genetic · ot_clinical` (evidence_pmids is `;`-joined; `ot_genetic`/`ot_clinical` are empty unless `--ot-overlay`, see below), sorted by `spec_adj` descending (with the artifact demotion applied to the sort score). **The gate is `--min-co` + the top-`--max` rank cut; `spec_adj` is a SORT key, not a filter** (`--min-specificity` defaults to `0.0` = off). A fixed specificity ratio is not objective across keywords/scan sizes and clips specific-but-pleiotropic genes that have real literature support (measured on atopic dermatitis: SPINK5 = Netherton/LEKTI barrier gene, PDE4B = crisaborole target, AHR = tapinarof MOA — all cut by a 0.05 ratio despite hundreds of co-papers). `--min-co` (default **10**) is the objective floor: a gene must co-occur with the keyword in ≥10 papers to count as literature-validated, an absolute count that is robust to keyword/scan size. Default thresholds: `--min-co 10`, `--min-gene-papers 10`, `--min-specificity 0.0` (off).
...
Alongside `genes.tsv`, the script always writes a sidecar `genes_all_scored.tsv` — **every scored candidate before the `min_co` filter**, same columns, sorted by `spec_adj`. Read it to inspect the full `spec_adj` distribution and the pleiotropic tail (high `co_papers`, low `spec_adj`) that the `--max` rank cut leaves out — the filtered `genes.tsv` alone can't show that. This is also where you audit false negatives: a gene with strong `co_papers` sitting just past the top-`--max` boundary is a lead to review, not a proven exclusion.
...
**Read spec_adj and co_papers as two axes, not one rank cut.** `spec_adj` is keyword *specificity*, not *relatedness* — a pleiotropic gene central to the keyword (IL4, STAT6, CD8A, and measured false negatives SPINK5/PDE4B/AHR) has a huge denominator, so `spec_adj` ranks it low and it can fall past the top-`--max` boundary even though it clears the `--min-co` floor. The `spec_adj` order is smooth (no natural elbow), so the `--max` boundary is a depth choice, not a truth line. So don't report the top-`--max` list as the whole answer: read `genes_all_scored.tsv` on both axes and label **two zones** — **core/specific** (high `spec_adj`) and **related-but-pleiotropic** (high `co_papers`, low `spec_adj`), surfacing the second zone's high-`co` genes explicitly. *Measured examples (IL4 co=8409/spec_adj~0.083, the smooth 0.018→0.008 band):* [`docs/method-rationale.md`](docs/method-rationale.md).
...
- Relax the floor: at scale, recall comes first, so lower `--min-co` to pass more through. `--min-specificity` is already off by default (`spec_adj` is sort-only), so the `spec_adj` sort still puts the most specific genes on top while the `--max` rank cut sets the depth.
